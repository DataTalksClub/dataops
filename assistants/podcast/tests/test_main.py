...
from main import create_collapsible_message, get_timestamp, is_allowed_chat, process_command, save_text_message, status_command
...
@pytest.mark.asyncio
async def test_process_command_reports_local_commit_without_pushing() -> None:
    update = Mock(spec=Update)
    update.effective_chat.id = 123456
    update.message = Mock(spec=Message)
    update.message.reply_text = AsyncMock()
    ...
    progress.finish = AsyncMock()
    progress.add_message = Mock()

    retrier = Mock()
    retrier.run_with_auto_retry = AsyncMock(return_value=(True, "abcdef1234567890", None))
    retrier.is_git_repo.return_value = True

    with (
        patch("main.is_allowed_chat", return_value=True),
        patch("main.ProgressTracker", return_value=progress),
        patch("main.SessionRetrier", return_value=retrier),
        patch("subprocess.run") as subprocess_run,
    ):
        await process_command(update, context)

    subprocess_run.assert_not_called()
    final_message = context.bot.send_message.await_args_list[-1].kwargs["text"]
    assert "Local commit detected: abcdef12. No push was performed." in final_message
