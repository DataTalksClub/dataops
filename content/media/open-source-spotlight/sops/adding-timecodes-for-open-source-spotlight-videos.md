---
id: sop.media.open-source-spotlight.adding-timecodes-for-open-source-spotlight-videos
aliases: []
title: "Adding timecodes for Open Source Spotlight videos"
summary: "Add timestamps to Open Source Spotlight videos so viewers can navigate demos, topics, and important discussion segments."
doc_type: sop
schema_version: 1
source: "Processes/Open-source spotlight/Adding timecodes for Open Source Spotlight videos.docx"
tags:
  - open-source-spotlight
systems:
  - loom
  - youtube
related_docs:
  - task-template.tasks.oss
---

# Adding timecodes for Open Source Spotlight videos

<!-- sop-section-start: summary -->
## Summary

- Purpose:
- Outcome:
- Trigger:
- Frequency:
<!-- sop-section-end -->

<!-- sop-section-start: prerequisites -->
## Prerequisites


- Access:
- Tools:
- Inputs:

When to use: After following the process doc for [Extracting Audio from (YouTube) Videos with VLC](../../video-youtube/sops/extracting-audio-from-youtube-videos-with-vlc.md).
<!-- sop-section-end -->

<!-- sop-section-start: procedure -->
## Procedure

<!-- sop-group-start: "Extracting audio from Youtube videos" -->
### Extracting audio from Youtube videos

<!-- sop-step-start id=1 -->
1.  Follow this process doc: [Extracting Audio from (YouTube) Videos with VLC](../../video-youtube/sops/extracting-audio-from-youtube-videos-with-vlc.md)
<!-- sop-step-end -->

<!-- sop-group-end -->

<!-- sop-group-start: "Title 2" -->
### Title 2

<!-- sop-step-start id=2 -->
2.  \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=3 -->
3.  \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=4 -->
4.  \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=5 -->
5.  \<insert image\>
<!-- sop-step-end -->

<!-- sop-group-end -->

<!-- sop-group-start: "Title 3" -->
### Title 3

<!-- sop-step-start id=6 -->
6.  \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=7 -->
7.  \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=8 -->
8.  \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=9 -->
9.  \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=10 -->
10. \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=11 -->
11. \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=12 -->
12. \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=13 -->
13. \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=14 -->
14. \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=15 -->
15. \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=16 -->
16. \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=17 -->
17. \<insert image\>
<!-- sop-step-end -->

<!-- sop-step-start id=18 -->
18. \<insert image\>

    Loom link:

    - TODO
<!-- sop-step-end -->

<!-- sop-group-end -->
<!-- sop-section-end -->

<!-- sop-section-start: validation -->
## Validation


-
<!-- sop-section-end -->

<!-- sop-section-start: troubleshooting -->
## Troubleshooting


-
<!-- sop-section-end -->

<!-- sop-section-start: references -->
## References


-
<!-- sop-section-end -->
