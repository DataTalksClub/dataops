...
from lambda_functions import doc_registry, sop_parse
...
def validate(repo_root: Path, content_root: Path | str = "content") -> list[str]:
    ...
    if registry is not None:
        ...
        violations.extend(validate_work_engine_seed_doc_ids(repo_root, registry))
    else:
...
def validate_work_engine_seed_doc_ids(repo_root: Path, registry: doc_registry.DocumentRegistry) -> list[str]:
    seed_path = repo_root / "work-engine" / "scripts" / "seed-templates.ts"
    if not seed_path.exists():
        return ["work-engine/scripts/seed-templates.ts: seed template source is required"]
