...
async def process_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        github_url = None
        if commit_hash and session_retrier.is_git_repo():
            subprocess.run(["git", "push"], cwd=REPO_PATH, capture_output=True, text=True, timeout=30)
