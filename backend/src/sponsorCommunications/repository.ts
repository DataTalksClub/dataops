...
async function migrateSuppressionRecord(
  client: DynamoDBDocumentClient,
  suppression: Record<string, unknown>,
  actorId: string,
  config: SendConfig,
  keyring: HmacKeyring,
): Promise<'migrated' | 'existing' | 'orphaned'> {
  const fromVersion = String(suppression.keyVersion);
  const contact = await getCrmRecord(client, 'contact', String(suppression.contactId));
  const address = Array.isArray(contact?.emails)
    ? contact.emails.flatMap((candidate) => {
      try {
        const canonical = normalizeEmail(candidate);
        return suppressionKey(fromVersion, canonical, keyring) === suppression.id
          ? [canonical]
          : [];
      } catch { return []; }
    })[0]
  ...
  const now = nowIso();
  if (!address) {
    const orphanId = String(suppression.id);
    try {
      await client.send(new TransactWriteCommand({
        TransactItems: [
          {
          ...
          },
          {
            ConditionCheck: {
              TableName: TABLE_SPONSOR_CRM,
              Key: itemKey('SPONSOR_SEND_CONFIG', 'CURRENT'),
              ConditionExpression: 'enabled = :false AND generation = :generation AND digest = :digest',
              ExpressionAttributeValues: { ':false': false, ':generation': config.generation, ':digest': config.digest },
            },
          },
          {
            Put: {
              TableName: TABLE_SPONSOR_CRM,
              Item: {
                ...itemKey('SUPPRESSION_MIGRATION_ORPHAN', orphanId),
                id: orphanId,
                recordType: 'suppression-migration-orphan',
                suppressionId: orphanId,
                ...
                createdAt: now,
                updatedAt: now,
                GSI1PK: 'SUPPRESSION_MIGRATION_ORPHAN',
                GSI1SK: `${now}#${orphanId}`,
              },
              ConditionExpression: 'attribute_not_exists(PK)',
            },
          },
          {
            Put: {
              TableName: TABLE_SPONSOR_CRM,
              Item: {
                ...itemKey('SPONSOR_COMM_AUDIT', `suppression-orphan#${orphanId}`),
                recordType: 'sponsor-communication-audit',
                action: 'suppression-migration-orphaned',
                actorId,
                suppressionId: orphanId,
                keyVersion: fromVersion,
                at: now,
      ...
      const existing = await getSponsorItem<Record<string, unknown>>(client, 'SUPPRESSION_MIGRATION_ORPHAN', orphanId);
      if (!existing) throw error;
    }
    return 'orphaned';
  }

  const newId = suppressionKey(config.hmacActiveVersion, address, keyring);
  for (let retry = 0; retry < 5; retry++) {
    const [oldRecord, existingNew, oldCount, newCount] = await Promise.all([
      getSponsorItem<Record<string, unknown>>(client, 'EMAIL_SUPPRESSION', String(suppression.id)),
      getSponsorItem<Record<string, unknown>>(client, 'EMAIL_SUPPRESSION', newId),
      getSponsorItem<Record<string, unknown>>(client, 'SUPPRESSION_VERSION_COUNT', fromVersion),
      getSponsorItem<Record<string, unknown>>(client, 'SUPPRESSION_VERSION_COUNT', config.hmacActiveVersion),
    ]);
    if (!oldRecord) return 'existing';
    if (!oldCount || Number(oldCount.liveCount) < 1) throw new Error('Old suppression count is inconsistent');
    const oldCountOperation = Number(oldCount.liveCount) === 1 ? {
      Delete: {
        TableName: TABLE_SPONSOR_CRM,
        Key: itemKey('SUPPRESSION_VERSION_COUNT', fromVersion),
      ...
      Update: {
        ...
        ConditionExpression: 'liveCount = :count',
        ExpressionAttributeValues: { ':minusOne': -1, ':count': Number(oldCount.liveCount), ':now': now },
      },
    };
    const newItems = existingNew ? [] : [
      {
        Put: {
          TableName: TABLE_SPONSOR_CRM,
          Item: {
            ...itemKey('EMAIL_SUPPRESSION', newId),
            ...oldRecord,
            PK: itemKey('EMAIL_SUPPRESSION', newId).PK,
            SK: itemKey('EMAIL_SUPPRESSION', newId).SK,
            id: newId,
      ...
      },
      {
        Update: {
          TableName: TABLE_SPONSOR_CRM,
          Key: itemKey('SUPPRESSION_VERSION_COUNT', config.hmacActiveVersion),
          UpdateExpression: 'ADD liveCount :one SET keyVersion = :version, recordType = :recordType, updatedAt = :now',
          ...(newCount ? {
            ConditionExpression: 'liveCount = :count',
            ExpressionAttributeValues: {
              ':one': 1, ':version': config.hmacActiveVersion, ':recordType': 'suppression-version-count', ':now': now, ':count': Number(newCount.liveCount),
            },
          } : {
    ...
    try {
        ...
        TransactItems: [
          ...
          },
          {
            ConditionCheck: {
              TableName: TABLE_SPONSOR_CRM,
              Key: itemKey('SPONSOR_SEND_CONFIG', 'CURRENT'),
              ConditionExpression: 'enabled = :false AND generation = :generation AND digest = :digest AND hmacActiveVersion = :active',
              ExpressionAttributeValues: {
                ':false': false, ':generation': config.generation, ':digest': config.digest, ':active': config.hmacActiveVersion,
              },
            },
          },
          {
            Delete: {
              TableName: TABLE_SPONSOR_CRM,
              Key: itemKey('EMAIL_SUPPRESSION', String(suppression.id)),
              ConditionExpression: 'revision = :revision AND keyVersion = :version AND #status = :active',
              ExpressionAttributeNames: { '#status': 'status' },
              ExpressionAttributeValues: {
                ':revision': Number(oldRecord.revision), ':version': fromVersion, ':active': 'active',
              },
            },
          },
          oldCountOperation,
          ...newItems,
          {
            Put: {
              TableName: TABLE_SPONSOR_CRM,
              Item: {
                ...itemKey('SPONSOR_COMM_AUDIT', `suppression-migrate#${suppression.id}`),
                recordType: 'sponsor-communication-audit',
                action: existingNew ? 'suppression-migration-deduplicated' : 'suppression-migrated',
                actorId,
                suppressionId: suppression.id,
                migratedSuppressionId: newId,
                fromVersion,
                toVersion: config.hmacActiveVersion,
                at: now,
                ttl: Math.floor(Date.parse(now) / 1000) + 365 * 24 * 60 * 60,
              },
              ConditionExpression: 'attribute_not_exists(PK)',
      ...
      return existingNew ? 'existing' : 'migrated';
    } catch (error) {
      if (!(error as Error).name.includes('Transaction') && !(error as Error).name.includes('Conditional')) throw error;
      if (retry === 4) throw new Error('Suppression migration conflicted repeatedly');
    }
  }
  throw new Error('Suppression migration failed');
}

export async function migrateSuppressions(
  client: DynamoDBDocumentClient,
  input: { actorId: string; fromVersion: string; limit?: number; cursor?: string },
  config: SendConfig,
  keyring: HmacKeyring,
): Promise<{ migrated: number; existing: number; orphaned: number; nextCursor: string | null; retired: boolean }> {
  if (config.enabled || input.fromVersion === config.hmacActiveVersion || !config.hmacAcceptedVersions.includes(input.fromVersion)) {
    throw new Error('Sending must be disabled with one previous accepted version before migration');
  }
  const limit = Math.min(Math.max(input.limit || 10, 1), 10);
  let exclusiveStartKey: Record<string, unknown> | undefined;
  if (input.cursor) {
    try { exclusiveStartKey = JSON.parse(Buffer.from(input.cursor, 'base64url').toString('utf8')); } catch { throw new Error('Invalid migration cursor'); }
  }
  const page = await client.send(new QueryCommand({
    TableName: TABLE_SPONSOR_CRM,
    IndexName: 'GSI-Communication',
    KeyConditionExpression: 'GSI1PK = :version',
    ExpressionAttributeValues: { ':version': `SUPPRESSION_VERSION#${input.fromVersion}` },
    Limit: limit,
    ExclusiveStartKey: exclusiveStartKey,
  }));
  const counts = { migrated: 0, existing: 0, orphaned: 0 };
  for (const suppression of page.Items || []) {
    const outcome = await migrateSuppressionRecord(client, suppression, input.actorId, config, keyring);
    counts[outcome]++;
  }
  let retired = false;
  if (!page.LastEvaluatedKey) {
    const oldCount = await getSponsorItem<Record<string, unknown>>(client, 'SUPPRESSION_VERSION_COUNT', input.fromVersion);
    const unresolved = await client.send(new QueryCommand({
      TableName: TABLE_SPONSOR_CRM,
      IndexName: 'GSI-Communication',
      KeyConditionExpression: 'GSI1PK = :version',
      ExpressionAttributeValues: { ':version': `SUPPRESSION_VERSION#${input.fromVersion}` },
    ...
    if (!oldCount && !(unresolved.Items || []).length) {
      await client.send(new UpdateCommand({
        TableName: TABLE_SPONSOR_CRM,
        Key: itemKey('SUPPRESSION_COVERAGE', 'CURRENT'),
        UpdateExpression: 'DELETE liveVersions :version SET updatedAt = :now',
        ExpressionAttributeValues: { ':version': new Set([input.fromVersion]), ':now': nowIso() },
      }));
      retired = true;
    }
  }
  return {
    ...counts,
    nextCursor: page.LastEvaluatedKey ? Buffer.from(JSON.stringify(page.LastEvaluatedKey)).toString('base64url') : null,
    retired,
  };
}
...
export async function reconcileSuppressionMigrationOrphan(
  client: DynamoDBDocumentClient,
  input: { id: string; actorId: string; reason: string },
): Promise<void> {
  const orphan = await getSponsorItem<Record<string, unknown>>(client, 'SUPPRESSION_MIGRATION_ORPHAN', input.id);
  if (!orphan) throw new Error('Suppression migration orphan not found');
  if (orphan.status === 'resolved') return;
  const suppression = await getSponsorItem<Record<string, unknown>>(client, 'EMAIL_SUPPRESSION', String(orphan.suppressionId));
  if (suppression) {
    await removeSuppression(client, {
      id: String(orphan.suppressionId),
      revision: Number(suppression.revision),
      actorId: input.actorId,
      reason: input.reason,
      allowProtected: true,
    });
  ...
  await client.send(new UpdateCommand({
    TableName: TABLE_SPONSOR_CRM,
    Key: itemKey('SUPPRESSION_MIGRATION_ORPHAN', input.id),
    UpdateExpression: 'SET #status = :resolved, resolvedBy = :actor, resolvedAt = :now, safeReason = :reason, revision = revision + :one',
    ConditionExpression: '#status = :unresolved',
    ExpressionAttributeNames: { '#status': 'status' },
    ExpressionAttributeValues: {
      ':resolved': 'resolved', ':unresolved': 'unresolved', ':actor': input.actorId,
      ':now': now, ':reason': input.reason.slice(0, 240), ':one': 1,
    },
  }));
}

export async function listSuppressionMigrationOrphans(
  client: DynamoDBDocumentClient,
  options: { limit?: number; cursor?: string } = {},
): Promise<{ items: Record<string, unknown>[]; nextCursor: string | null }> {
  ...
  if (options.cursor) {
    ...
    } catch { throw new Error('Invalid orphan cursor'); }
  }
  const page = await client.send(new QueryCommand({
    TableName: TABLE_SPONSOR_CRM,
    IndexName: 'GSI-Communication',
    KeyConditionExpression: 'GSI1PK = :pk',
    ExpressionAttributeValues: { ':pk': 'SUPPRESSION_MIGRATION_ORPHAN' },
    ScanIndexForward: false,
    Limit: limit,
    ExclusiveStartKey: exclusiveStartKey,
  }));
  return {
    items: (page.Items || []).map((item) => ({
      id: String(item.id),
      keyVersion: String(item.keyVersion),
