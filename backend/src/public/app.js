(function () {
  ...
  function renderSponsors() {
    clearApp();
    app.innerHTML = '<section class="page-header"><div><h2>Sponsor CRM</h2><p class="page-subtitle">Organizations, contacts, bookings, and next actions</p></div><div><button id="crm-add-org">Add sponsor</button> <button class="btn btn-primary" id="crm-add-booking">Add booking</button></div></section>' +
      '<section class="form-row"><label>Search sponsors <input id="crm-search" type="search"></label><label>Show <select id="crm-active"><option value="true">Active</option><option value="false">Archived</option><option value="">All</option></select></label><label>Booking status <select id="crm-status"><option value="">All</option><option>inquiry</option><option>held</option><option>confirmed</option><option>materials-pending</option><option>materials-ready</option><option>scheduled</option><option>published</option><option>performance-due</option><option>complete</option><option>cancelled</option></select></label></section>' +
      '<p id="crm-status-message" role="status">Loading sponsor CRM…</p><div class="crm-grid"><section><h3>Sponsors</h3><div id="crm-organizations" class="crm-list"></div></section><section><h3>Bookings</h3><div id="crm-bookings" class="crm-list"></div><div id="crm-detail"></div></section></div>' +
      '<dialog id="crm-org-dialog"><form method="dialog" class="crm-form" id="crm-org-form"><h3>Sponsor organization</h3><label>Name <input name="displayName" required maxlength="200"></label><label>Source key <input name="sourceKey" maxlength="200"></label><label>Private notes <textarea name="notes" maxlength="2000"></textarea></label><p role="alert"></p><div class="form-actions"><button value="cancel">Cancel</button><button value="default" class="btn btn-primary">Save sponsor</button></div></form></dialog>' +
      '<dialog id="crm-contact-dialog"><form method="dialog" class="crm-form" id="crm-contact-form"><h3>Contact</h3><input type="hidden" name="organizationId"><label>Name <input name="name" required></label><label>Email <input name="email" type="email" required></label><label>Role <input name="role"></label><label><input name="primary" type="checkbox"> Primary contact</label><label>Private notes <textarea name="notes"></textarea></label><p role="alert"></p><div class="form-actions"><button value="cancel">Cancel</button><button value="default" class="btn btn-primary">Save contact</button></div></form></dialog>' +
      '<dialog id="crm-booking-dialog"><form method="dialog" class="crm-form" id="crm-booking-form"><h3>Sponsor booking</h3><input type="hidden" name="id"><input type="hidden" name="version"><label>Sponsor <select name="organizationId" required></select></label><label>Primary contact <select name="primaryContactId"><option value="">No contact</option></select></label><label>Slot type <select name="slotType"><option>main</option><option>secondary</option><option>standalone</option></select></label><label>Status <select name="status"><option>inquiry</option><option>held</option><option>confirmed</option><option>materials-pending</option><option>materials-ready</option><option>scheduled</option><option>published</option><option>performance-due</option><option>complete</option><option>cancelled</option></select></label><label>Publication date <input name="plannedPublicationDate" type="date"></label><label>Material deadline <input name="materialDeadline" type="date"></label><label>Next action <input name="nextActionDate" type="date"></label><label>Schedule entry ID <input name="scheduleEntryId"></label><label>Newsletter bundle ID <input name="bundleId"></label><label>Required link <input name="requiredLinkUrl" type="url"></label><label>Private artifact URLs <textarea name="artifactUrls" placeholder="One HTTPS URL per line"></textarea></label><label>Operator notes <textarea name="notes"></textarea></label><label>Status note <input name="historyNote"></label><p role="alert"></p><div class="form-actions"><button value="cancel">Cancel</button><button value="default" class="btn btn-primary">Save booking</button></div></form></dialog>';
    var organizations=[],contacts=[],bookings=[], status=document.getElementById('crm-status-message');
    function fail(message){status.textContent=message;}
    function options(){document.querySelector('#crm-booking-form [name="organizationId"]').innerHTML=organizations.filter(function(o){return !o.archivedAt;}).map(function(o){return '<option value="'+escapeHtml(o.id)+'">'+escapeHtml(o.displayName)+'</option>';}).join('');}
    function draw(){var search=document.getElementById('crm-search').value.toLowerCase(),active=document.getElementById('crm-active').value,bookingStatus=document.getElementById('crm-status').value;var shown=organizations.filter(function(o){return (!search||String(o.displayName).toLowerCase().includes(search))&&(!active||String(!o.archivedAt)===active);});document.getElementById('crm-organizations').innerHTML=shown.length?shown.map(function(o){var count=bookings.filter(function(b){return b.organizationId===o.id&&b.status!=='cancelled'&&b.status!=='complete';}).length;return '<article class="crm-item"><header><strong>'+escapeHtml(o.displayName)+'</strong><span>'+count+' current</span></header><p>'+escapeHtml(o.archivedAt?'Archived':'Active')+'</p><button data-add-contact="'+escapeHtml(o.id)+'">Add contact</button> <button data-view-org="'+escapeHtml(o.id)+'">View sponsor</button>'+(o.archivedAt?'':' <button data-archive-org="'+escapeHtml(o.id)+'">Archive</button>')+'</article>';}).join(''):'<div class="empty-state"><h3>No sponsors found</h3><p>Adjust filters or add the first sponsor.</p></div>';var visible=bookings.filter(function(b){return !bookingStatus||b.status===bookingStatus;});document.getElementById('crm-bookings').innerHTML=visible.length?visible.map(function(b){var org=organizations.find(function(o){return o.id===b.organizationId;});return '<article class="crm-item"><header><strong>'+escapeHtml(org?org.displayName:'Unknown sponsor')+'</strong><span class="crm-badge">'+escapeHtml(b.status)+'</span></header><p>'+escapeHtml(b.plannedPublicationDate||'Date not set')+' · next action '+escapeHtml(b.nextActionDate||'not set')+'</p><button data-view-booking="'+escapeHtml(b.id)+'">Open booking</button> <button data-edit-booking="'+escapeHtml(b.id)+'">Edit</button></article>';}).join(''):'<div class="empty-state"><h3>No bookings</h3><p>Create a booking or adjust the status filter.</p></div>';}
    function refresh(){status.textContent='Loading sponsor CRM…';return Promise.all([api.sponsorCrm.list('organizations'),api.sponsorCrm.list('contacts'),api.sponsorCrm.list('bookings')]).then(function(results){organizations=results[0].items||[];contacts=results[1].items||[];bookings=results[2].items||[];options();draw();status.textContent='Sponsor CRM ready.';var requested=parseHash().params.bookingId;if(requested)showBooking(requested);}).catch(function(error){fail('Could not load sponsor CRM: '+error.message);document.getElementById('crm-organizations').innerHTML='<div class="error-state">Retry by reopening Sponsors.</div>';});}
    function showBooking(id){var booking=bookings.find(function(b){return b.id===id;});if(!booking)return;Promise.all([api.sponsorCrm.history(id)]).then(function(results){var org=organizations.find(function(o){return o.id===booking.organizationId;}),contact=contacts.find(function(c){return c.id===booking.primaryContactId;});document.getElementById('crm-detail').innerHTML='<article class="crm-item"><h3>Booking detail</h3><p><strong>'+escapeHtml(org?org.displayName:'Unknown sponsor')+'</strong> · '+escapeHtml(booking.status)+'</p><p>Primary contact: '+escapeHtml(contact?contact.name:'Not set')+'</p><p>Publication '+escapeHtml(booking.plannedPublicationDate||'not set')+' · material deadline '+escapeHtml(booking.materialDeadline||'not set')+' · next action '+escapeHtml(booking.nextActionDate||'not set')+'</p><p>Newsletter bundle: '+escapeHtml(booking.bundleId||'Not linked')+' · schedule entry: '+escapeHtml(booking.scheduleEntryId||'Not linked')+'</p><div class="crm-timeline">'+((results[0].items||[]).map(function(event){return '<div><strong>'+escapeHtml(String(event.oldStatus||'created'))+' → '+escapeHtml(event.newStatus)+'</strong><br><small>'+escapeHtml(event.createdAt)+' · '+escapeHtml(event.actorId)+'</small>'+(event.note?'<p>'+escapeHtml(event.note)+'</p>':'')+'</div>';}).join('')||'<p>No status changes yet.</p>')+'</div></article>';});}
    function openBooking(item){var form=document.getElementById('crm-booking-form');form.reset();Array.from(form.elements).forEach(function(field){if(item&&field.name&&item[field.name]!=null)field.value=Array.isArray(item[field.name])?item[field.name].join('\n'):item[field.name];});var orgId=item?item.organizationId:form.elements.organizationId.value;form.elements.primaryContactId.innerHTML='<option value="">No contact</option>'+contacts.filter(function(c){return c.organizationId===orgId&&!c.archivedAt;}).map(function(c){return '<option value="'+escapeHtml(c.id)+'">'+escapeHtml(c.name)+'</option>';}).join('');if(item&&item.primaryContactId)form.elements.primaryContactId.value=item.primaryContactId;document.getElementById('crm-booking-dialog').showModal();}
    document.getElementById('crm-search').oninput=draw;document.getElementById('crm-active').onchange=draw;document.getElementById('crm-status').onchange=draw;document.getElementById('crm-add-org').onclick=function(){document.getElementById('crm-org-form').reset();document.getElementById('crm-org-dialog').showModal();};document.getElementById('crm-add-booking').onclick=function(){openBooking(null);};
    document.getElementById('crm-organizations').onclick=function(event){var contact=event.target.getAttribute('data-add-contact'),archive=event.target.getAttribute('data-archive-org'),view=event.target.getAttribute('data-view-org');if(contact){var form=document.getElementById('crm-contact-form');form.reset();form.elements.organizationId.value=contact;document.getElementById('crm-contact-dialog').showModal();}if(archive&&confirm('Archive this sponsor?'))api.sponsorCrm.archive('organizations',archive).then(refresh).catch(function(error){fail('Could not archive sponsor: '+error.message);});if(view){var org=organizations.find(function(o){return o.id===view;});document.getElementById('crm-detail').innerHTML='<article class="crm-item"><h3>'+escapeHtml(org.displayName)+'</h3><h4>Contacts</h4>'+contacts.filter(function(c){return c.organizationId===view;}).map(function(c){return '<p><strong>'+escapeHtml(c.name)+'</strong> · '+escapeHtml(c.role||'Contact')+(c.primary?' · primary':'')+'</p>';}).join('')+'<h4>Bookings</h4><p>'+bookings.filter(function(b){return b.organizationId===view;}).length+' total bookings</p></article>';}};
    document.getElementById('crm-bookings').onclick=function(event){var view=event.target.getAttribute('data-view-booking'),edit=event.target.getAttribute('data-edit-booking');if(view)showBooking(view);if(edit)openBooking(bookings.find(function(b){return b.id===edit;}));};
    document.querySelector('#crm-booking-form [name="organizationId"]').onchange=function(event){var form=document.getElementById('crm-booking-form');form.elements.primaryContactId.innerHTML='<option value="">No contact</option>'+contacts.filter(function(c){return c.organizationId===event.target.value&&!c.archivedAt;}).map(function(c){return '<option value="'+escapeHtml(c.id)+'">'+escapeHtml(c.name)+'</option>';}).join('');};
    document.getElementById('crm-org-form').onsubmit=function(event){event.preventDefault();var form=event.target,data=Object.fromEntries(new FormData(form));api.sponsorCrm.create('organizations',data).then(function(){document.getElementById('crm-org-dialog').close();return refresh();}).catch(function(error){form.querySelector('[role="alert"]').textContent=error.message;});};
    document.getElementById('crm-contact-form').onsubmit=function(event){event.preventDefault();var form=event.target,data=Object.fromEntries(new FormData(form));data.emails=[data.email];delete data.email;data.primary=form.elements.primary.checked;api.sponsorCrm.create('contacts',data).then(function(){document.getElementById('crm-contact-dialog').close();return refresh();}).catch(function(error){form.querySelector('[role="alert"]').textContent=error.message;});};
    document.getElementById('crm-booking-form').onsubmit=function(event){event.preventDefault();var form=event.target,data=Object.fromEntries(Array.from(new FormData(form)).filter(function(pair){return pair[1]!=='';}));if(data.version)data.version=Number(data.version);if(data.artifactUrls)data.artifactUrls=String(data.artifactUrls).split(/\n/).filter(Boolean);var id=data.id;delete data.id;var request=id?api.sponsorCrm.update('bookings',id,data):api.sponsorCrm.create('bookings',data);request.then(function(){document.getElementById('crm-booking-dialog').close();return refresh();}).catch(function(error){form.querySelector('[role="alert"]').textContent=(error.message.indexOf('Schedule')>=0?'Booking conflict: ':'Could not save booking: ')+error.message;});};
    document.querySelectorAll('#crm-org-dialog [value="cancel"],#crm-contact-dialog [value="cancel"],#crm-booking-dialog [value="cancel"]').forEach(function(button){button.onclick=function(event){event.preventDefault();button.closest('dialog').close();};});
    refresh();
  }
  ...
  function renderCalendar(){clearApp();app.innerHTML='<section class="page-header"><div><h2>Operations calendar</h2><p class="page-subtitle">Europe/Berlin · Monday–Sunday · ISO weeks</p></div><button id="calendar-add" class="btn btn-primary">Add activity</button></section><section class="form-row" aria-label="Calendar controls"><button id="calendar-prev">Previous</button><button id="calendar-today">Today</button><button id="calendar-next">Next</button><label>View <select id="calendar-view"><option value="month">Month</option><option value="week">Week</option></select></label><label><input id="calendar-activities" type="checkbox" checked> Activities</label><label><input id="calendar-public" type="checkbox" checked> Public holidays</label><label><input id="calendar-school" type="checkbox" checked> School holidays</label></section><p id="calendar-message" role="status"></p><div id="calendar-alerts" aria-live="polite"></div><div id="calendar-grid"></div><dialog id="calendar-dialog"><form class="crm-form"><h3>Activity</h3><input name="id" type="hidden"><input name="version" type="hidden"><label>Title <input name="title" required maxlength="200"></label><label>Type <select name="activityType"><option value="podcast-live">Podcast live</option><option value="podcast-release">Podcast release</option><option>webinar</option><option>workshop</option><option value="book-of-the-week">Book of the week</option><option>course</option><option>cohort</option><option>other</option></select></label><label>Status <select name="status"><option>tentative</option><option>confirmed</option><option>announced</option><option>published</option><option>cancelled</option></select></label><label>Start <input name="startDate" type="date" required></label><label>End <input name="endDate" type="date" required></label><label>Bundle ID <input name="bundleId"></label><label>Notes <textarea name="notes" maxlength="2000"></textarea></label><p role="alert"></p><button type="button" data-close>Cancel</button><button class="btn btn-primary">Save</button></form></dialog>';var cursor=new Date(),items=[],holidays=[],dialog=document.getElementById('calendar-dialog'),form=dialog.querySelector('form');function iso(d){return d.toISOString().slice(0,10)}function bounds(){var view=document.getElementById('calendar-view').value,d=new Date(cursor);if(view==='week'){var day=(d.getUTCDay()+6)%7,d1=new Date(d);d1.setUTCDate(d.getUTCDate()-day);var d2=new Date(d1);d2.setUTCDate(d1.getUTCDate()+6);return[iso(d1),iso(d2)]}return[d.getUTCFullYear()+'-'+String(d.getUTCMonth()+1).padStart(2,'0')+'-01',iso(new Date(Date.UTC(d.getUTCFullYear(),d.getUTCMonth()+1,0)))]}function load(){var b=bounds();document.getElementById('calendar-message').textContent='Loading '+b[0]+' – '+b[1]+'…';api.calendar.list({from:b[0],to:b[1]}).then(function(r){items=r.items||[];holidays=r.holidays||[];document.getElementById('calendar-alerts').innerHTML=(r.alerts||[]).map(function(a){return'<p class="error-banner">'+escapeHtml(a.reasonCode)+' · '+escapeHtml(a.severity)+'</p>'}).join('');render();document.getElementById('calendar-message').textContent=(r.holidayMetadata&&r.holidayMetadata.outOfHorizon?'Holiday source is outside its verified horizon. ':'')+'Calendar ready.'}).catch(function(e){document.getElementById('calendar-message').textContent='Could not load calendar: '+e.message})}function render(){var showA=document.getElementById('calendar-activities').checked,showP=document.getElementById('calendar-public').checked,showS=document.getElementById('calendar-school').checked,b=bounds(),start=new Date(b[0]+'T00:00:00Z'),end=new Date(b[1]+'T00:00:00Z'),html='<div class="calendar-grid" style="display:grid;grid-template-columns:repeat(7,minmax(8rem,1fr));gap:.35rem;overflow:auto">';for(var d=new Date(start);d<=end;d.setUTCDate(d.getUTCDate()+1)){var date=iso(d),dayItems=showA?items.filter(function(i){return i.startKey.slice(0,10)<=date&&i.endKey.slice(0,10)>=date}):[],dayHolidays=holidays.filter(function(h){return h.startDate<=date&&h.endDate>=date&&((h.kind==='berlin-public-holiday'&&showP)||(h.kind!=='berlin-public-holiday'&&showS))});html+='<section style="min-height:8rem;border:1px solid var(--border-color);padding:.5rem" aria-label="'+date+'"><strong>'+date.slice(8)+' '+d.toLocaleDateString('en',{weekday:'short',timeZone:'UTC'})+'</strong>'+dayHolidays.map(function(h){return'<p class="crm-badge" style="display:block">'+escapeHtml(h.name)+(h.name==='Summer holidays'?' ☀':'')+'</p>'}).join('')+dayItems.map(function(i){return'<button data-calendar-edit="'+i.id+'" style="display:block;width:100%;margin:.2rem 0">'+escapeHtml(i.title)+'</button>'}).join('')+'</section>'}document.getElementById('calendar-grid').innerHTML=html+'</div>'}document.getElementById('calendar-prev').onclick=function(){cursor.setUTCDate(1);cursor.setUTCMonth(cursor.getUTCMonth()+(document.getElementById('calendar-view').value==='week'?-0.25:-1));if(document.getElementById('calendar-view').value==='week')cursor.setUTCDate(cursor.getUTCDate()-7);load()};document.getElementById('calendar-next').onclick=function(){document.getElementById('calendar-view').value==='week'?cursor.setUTCDate(cursor.getUTCDate()+7):cursor.setUTCMonth(cursor.getUTCMonth()+1);load()};document.getElementById('calendar-today').onclick=function(){cursor=new Date();load()};document.querySelectorAll('#calendar-view,#calendar-activities,#calendar-public,#calendar-school').forEach(function(el){el.onchange=el.id==='calendar-view'?load:render});document.getElementById('calendar-add').onclick=function(){form.reset();var date=iso(cursor);form.elements.startDate.value=date;form.elements.endDate.value=date;dialog.showModal()};document.getElementById('calendar-grid').onclick=function(e){var item=items.find(function(i){return i.id===e.target.getAttribute('data-calendar-edit')});if(!item)return;form.reset();Object.keys(item).forEach(function(k){if(form.elements[k])form.elements[k].value=item[k]||''});dialog.showModal()};form.querySelector('[data-close]').onclick=function(){dialog.close()};form.onsubmit=function(e){e.preventDefault();var value=Object.fromEntries(Array.from(new FormData(form)).filter(function(p){return p[1]!==''})),id=value.id;delete value.id;value.allDay=true;value.timeZone='Europe/Berlin';if(value.version)value.version=Number(value.version);(id?api.calendar.update(id,value):api.calendar.create(value)).then(function(){dialog.close();load()}).catch(function(err){form.querySelector('[role="alert"]').textContent=err.message})};load()}

  function renderMailingExports(){
    clearApp();
    app.innerHTML='<section class="page-header"><div><h2>Mailing-list exports</h2><p class="page-subtitle">Private account-wide audiences archives · one completed Mailchimp export per 24 hours</p></div><button id="mailing-refresh">Refresh</button></section><p id="mailing-message" role="status" aria-live="polite">Loading export configurations…</p><div id="mailing-configs" class="mailing-export-grid"></div><section class="card"><h3>Run history</h3><div id="mailing-history"></div></section>';
    var message=document.getElementById('mailing-message'),configsRoot=document.getElementById('mailing-configs'),historyRoot=document.getElementById('mailing-history');
    function time(value){return value?new Date(value).toLocaleString():'Not yet'}
    function next(run){var copy={wait:'Wait for the provider, then refresh or advance this run.',retry:'Retry this run with the same key.','fix-authorization':'Fix provider authorization, then retry.','fix-storage':'Fix private storage access, then retry.','fix-task-link':'Fix the recurring-task link; the archive remains stored.',download:'Archive ready for a private five-minute download.'};return run&&run.nextAction?copy[run.nextAction]:'Start the first account-wide audiences export.'}
    function card(config,run){var state=run?run.status:'empty',continuing=run&&['requested','pending','failed'].includes(run.status)&&run.runKey,runKey=continuing?run.runKey:new Date().toISOString().slice(0,10);return'<article class="mailing-export-card" data-export-state="'+escapeHtml(state)+'"><header><div><h3>'+escapeHtml(config.account)+'</h3><p>'+escapeHtml(config.scopeLabel)+' · '+escapeHtml(config.provider)+'</p></div><span class="crm-badge">'+escapeHtml(state)+'</span></header><dl><div><dt>Requested</dt><dd>'+escapeHtml(time(run&&run.requestedAt))+'</dd></div><div><dt>Completed</dt><dd>'+escapeHtml(time(run&&run.completedAt))+'</dd></div><div><dt>Recurring task</dt><dd>'+escapeHtml(run&&run.taskId?(run.taskLinkStatus||'pending')+' · '+run.taskId:'Not configured')+'</dd></div><div><dt>Artifact</dt><dd>'+escapeHtml(run&&run.artifactId||'Not created')+'</dd></div></dl>'+(run&&run.errorMessage?'<p class="error-banner" role="alert"><strong>'+escapeHtml(run.errorCode)+'</strong> · '+escapeHtml(run.errorMessage)+'</p>':'')+'<p>'+escapeHtml(next(run))+(run&&run.retryAfter?' Earliest retry: '+escapeHtml(time(run.retryAfter))+'.':'')+'</p><div class="mailing-export-actions"><button class="btn btn-primary" data-mailing-run="'+escapeHtml(config.id)+'" data-run-key="'+escapeHtml(runKey)+'">'+(run&&run.status!=='completed'?'Advance / retry':'Start daily export')+'</button>'+(run&&run.status==='completed'&&run.artifactId?'<button data-mailing-download="'+escapeHtml(run.artifactId)+'">Download ZIP</button>':'')+'</div></article>'}
    function load(){message.textContent='Loading export configurations…';return api.mailingExports.list().then(function(result){var configs=result.configs||[],runs=result.exports||[];configsRoot.innerHTML=configs.length?configs.map(function(config){return card(config,runs.find(function(run){return run.configId===config.id}))}).join(''):'<div class="empty-state" data-export-state="no-config"><h3>No export configurations</h3><p>Add an enabled provider configuration through the approved deploy mechanism. No secrets belong in the portal.</p></div>';historyRoot.innerHTML=runs.length?runs.map(function(run){return'<article class="mailing-export-history"><strong>'+escapeHtml(run.account)+' · '+escapeHtml(run.status)+'</strong><span>'+escapeHtml(run.scopeLabel)+' · requested '+escapeHtml(time(run.requestedAt))+'</span></article>'}).join(''):'<div class="empty-state" data-export-state="empty"><h3>No export runs yet</h3><p>Start an enabled configuration to create durable history.</p></div>';message.textContent=configs.length?configs.length+' export configuration'+(configs.length===1?'':'s')+' loaded.':'No export configurations are enabled.'}).catch(function(error){message.textContent='Could not load mailing-list exports: '+error.message;configsRoot.innerHTML='<div class="error-state" data-export-state="failure">Retry with Refresh. No provider or archive details were exposed.</div>';historyRoot.innerHTML=''})}
    document.getElementById('mailing-refresh').onclick=load;
    configsRoot.onclick=function(event){var run=event.target.closest('[data-mailing-run]'),download=event.target.closest('[data-mailing-download]');if(run){run.disabled=true;run.setAttribute('aria-busy','true');message.textContent='Requesting or advancing the durable export run…';api.mailingExports.run(run.getAttribute('data-mailing-run'),run.getAttribute('data-run-key')).then(load).catch(function(error){message.textContent='Could not advance export: '+error.message;run.disabled=false;run.removeAttribute('aria-busy')})}if(download){download.disabled=true;api.mailingExports.download(download.getAttribute('data-mailing-download')).then(function(result){var link=document.createElement('a');link.href=result.downloadUrl;link.target='_blank';link.rel='noopener';link.click();message.textContent='Private download prepared. The link expires in five minutes.'}).catch(function(error){message.textContent='Could not prepare download: '+error.message}).finally(function(){download.disabled=false})}};
    load();
  }
  ...
  var routes = {
    ...
    '#/mailing-exports': renderMailingExports
  };
