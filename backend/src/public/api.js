(function () {
  ...
  window.api = {
    ...
    mailingExports:{list:function(){return fetch('/api/mailing-exports',{headers:getAuthHeaders()}).then(handleResponse);},run:function(configId,runKey){return fetch('/api/mailing-exports/run',{method:'POST',headers:getAuthHeaders(),body:JSON.stringify({configId:configId,runKey:runKey})}).then(handleResponse);},download:function(artifactId){return fetch('/api/artifacts/'+encodeURIComponent(artifactId)+'/download',{headers:getAuthHeaders()}).then(handleResponse);}},
