...
export type ImportRow = Record<string, string | number | undefined> & { transactionDate: string; counterparty: string; description: string; amount: string; currency: string; sourceType: string; sourceKey: string; sourceCoordinate?: string };
...
export async function parseWorkbook(file: string): Promise<{ rows: ImportRow[]; rejected: Rejection[]; warnings: Rejection[]; counts: Record<string,{accepted:number;rejected:number}> }> {
  const workbook=new ExcelJS.Workbook();await workbook.xlsx.readFile(file); const rows:ImportRow[]=[];const rejected:Rejection[]=[];const warnings:Rejection[]=[];const counts:Record<string,{accepted:number;rejected:number}>={};
  for(const worksheet of workbook.worksheets){const sheet=worksheet.name;counts[sheet]={accepted:0,rejected:0};const matrix:unknown[][]=[];worksheet.eachRow({includeEmpty:true},row=>matrix.push(Array.from({length:Math.max(row.cellCount,worksheet.columnCount)},(_,i)=>cellValue(row.getCell(i+1).value))));if(!matrix.length)continue;
    ...
    for(let i=headerIndex+1;i<matrix.length;i++){const raw=matrix[i] as unknown[];if(!raw||!raw.slice(0,width).some(v=>v!==undefined&&v!==null&&String(v).trim()))continue;const value:Record<string,unknown>={};map.forEach((k,j)=>{if(k)value[k]=raw[j]});
      ...
      for(const k of ['paidDate','amountEur','statementRef','comment','entryType','subtype','period','category'])if(value[k]!=null&&String(value[k]).trim()){if(k==='paidDate'||k==='amountEur'){const normalized=k==='paidDate'?isoDate(value[k]):decimal(value[k]);if(normalized)record[k]=normalized;else warnings.push({sheet,row:i+1,reason:k==='paidDate'?'OPTIONAL_PAID_DATE_UNMAPPED':'OPTIONAL_AMOUNT_EUR_UNMAPPED'});}else record[k]=String(value[k]).trim();}
      Object.defineProperty(record,'sourceCoordinate',{value:`${sheet}:${i+1}`,enumerable:false});
  ...
  } return {rows,rejected,warnings,counts};
}
export async function writeRows(rows: ImportRow[], options:{api:string;sessionToken:string;concurrency?:number;ratePerMinute?:number;fetcher?:typeof fetch;sleep?:(ms:number)=>Promise<void>;now?:()=>number}) {
  const fetcher=options.fetcher||fetch, sleep=options.sleep||(ms=>new Promise(r=>setTimeout(r,ms))), now=options.now||Date.now, queue=[...rows];let created=0,duplicates=0;
  const rate=Math.max(1,Math.min(options.ratePerMinute||50,60)), interval=60000/rate;let nextAt=now();
  const sessionAuthorization=`Bearer ${options.sessionToken}`;
  async function pace(){const scheduled=Math.max(nextAt,now());nextAt=scheduled+interval;const delay=scheduled-now();if(delay>0)await sleep(delay);}
  async function worker(){for(;;){const row=queue.shift();if(!row)return;let response:Response|undefined;for(let attempt=0;attempt<6;attempt++){await pace();response=await fetcher(`${options.api.replace(/\/$/,'')}/api/bookkeeping/transactions`,{method:'POST',headers:{'content-type':'application/json',authorization:sessionAuthorization},body:JSON.stringify(row)});if(![429,500,502,503,504].includes(response.status))break;const retryAfter=Number(response.headers.get('retry-after'));const backoff=Number.isFinite(retryAfter)&&retryAfter>0?retryAfter*1000:Math.min(1000*2**attempt,30000);await sleep(backoff);}if(!response?.ok){let reason=`HTTP_${response?.status||0}`;try{const failure=await response.json() as {fields?:unknown};const allowed=new Set(['transactionDate','paidDate','counterparty','description','amount','currency','amountEur','quantity','statementRef','comment','entryType','subtype','period','category','sourceType','sourceKey']);if(Array.isArray(failure.fields)){const fields=failure.fields.filter((v):v is string=>typeof v==='string'&&allowed.has(v));if(fields.length)reason=`VALIDATION_${fields.join('_').toUpperCase()}`;}}catch{}throw new Error(`Import stopped: ${row.sourceCoordinate||'unknown-row'} ${reason} HTTP ${response?.status||0}`);}response.status===200?duplicates++:created++;}}
...
async function main(){const args=process.argv.slice(2);const file=args.find(a=>!a.startsWith('--'))||'.tmp/bookkeeping.xlsx';const parsed=await parseWorkbook(file);console.log(JSON.stringify({mode:args.includes('--write')?'write':'dry-run',counts:parsed.counts,rejected:parsed.rejected,warnings:parsed.warnings}));if(!args.includes('--write'))return;
  const api=process.env.BOOKKEEPING_API_URL, env=process.env.BOOKKEEPING_TARGET_ENV, confirm=args.find(a=>a.startsWith('--confirm='))?.slice(10);if(!api||!env||confirm!==env)throw new Error('Write requires API URL, target environment, and matching --confirm');
  const session=process.env.BOOKKEEPING_SESSION_TOKEN;if(!session)throw new Error('Write requires BOOKKEEPING_SESSION_TOKEN');
  const verified=await writeRows(parsed.rows,{api,sessionToken:session,concurrency:Number(process.env.BOOKKEEPING_IMPORT_CONCURRENCY||4),ratePerMinute:Number(process.env.BOOKKEEPING_IMPORT_RATE_PER_MINUTE||50)});
