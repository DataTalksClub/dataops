...
export async function writeRows(rows: ImportRow[], options:{api:string;secret:string;portalUsername?:string;portalPassword?:string;concurrency?:number;ratePerMinute?:number;fetcher?:typeof fetch;sleep?:(ms:number)=>Promise<void>;now?:()=>number}) {
  ...
  const portalAuthorization=options.portalUsername&&options.portalPassword?`Basic ${Buffer.from(`${options.portalUsername}:${options.portalPassword}`).toString('base64')}`:undefined;
...
async function main(){const args=process.argv.slice(2);const file=args.find(a=>!a.startsWith('--'))||'.tmp/bookkeeping.xlsx';const parsed=await parseWorkbook(file);console.log(JSON.stringify({mode:args.includes('--write')?'write':'dry-run',counts:parsed.counts,rejected:parsed.rejected}));if(!args.includes('--write'))return;
  ...
  const portalUsername=process.env.BOOKKEEPING_PORTAL_USERNAME,portalPassword=process.env.BOOKKEEPING_PORTAL_PASSWORD;if(Boolean(portalUsername)!==Boolean(portalPassword))throw new Error('Portal username and password must be provided together');
  const verified=await writeRows(parsed.rows,{api,secret,portalUsername,portalPassword,concurrency:Number(process.env.BOOKKEEPING_IMPORT_CONCURRENCY||4),ratePerMinute:Number(process.env.BOOKKEEPING_IMPORT_RATE_PER_MINUTE||50)});
  const session=process.env.BOOKKEEPING_SESSION_TOKEN;if(!session&&!portalUsername)throw new Error('Write verification requires BOOKKEEPING_SESSION_TOKEN or portal credentials');const verificationAuthorization=portalUsername&&portalPassword?`Basic ${Buffer.from(`${portalUsername}:${portalPassword}`).toString('base64')}`:`Bearer ${session}`;const response=await fetch(`${api.replace(/\/$/,'')}/api/bookkeeping/transactions`,{headers:{authorization:verificationAuthorization}});if(!response.ok)throw new Error(`Verification stopped: HTTP ${response.status}`);const listed=await response.json() as {items:ImportRow[]};const imported=new Set(listed.items.filter(i=>i.sourceType==='xlsx-import').map(i=>i.sourceKey));if(parsed.rows.some(r=>!imported.has(r.sourceKey)))throw new Error('Verification failed: imported source keys missing');console.log(JSON.stringify({verified}));}
