...
export async function writeRows(rows: ImportRow[], options:{api:string;secret:string;concurrency?:number;ratePerMinute?:number;fetcher?:typeof fetch;sleep?:(ms:number)=>Promise<void>;now?:()=>number}) {
...
async function main(){const args=process.argv.slice(2);const file=args.find(a=>!a.startsWith('--'))||'.tmp/bookkeeping.xlsx';const parsed=await parseWorkbook(file);console.log(JSON.stringify({mode:args.includes('--write')?'write':'dry-run',counts:parsed.counts,rejected:parsed.rejected}));if(!args.includes('--write'))return;
  ...
  const verified=await writeRows(parsed.rows,{api,secret,concurrency:Number(process.env.BOOKKEEPING_IMPORT_CONCURRENCY||4),ratePerMinute:Number(process.env.BOOKKEEPING_IMPORT_RATE_PER_MINUTE||50)});
